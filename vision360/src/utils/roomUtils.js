 ...
 */
export function buildRoomData(rooms, overrides, events, floorKey) {
    const customRooms = Object.entries(overrides)
        .filter(([id, data]) => data.isNew && data.floorKey === floorKey && !data.hidden)
        .map(([id, data]) => ({ id, ...data }));

    const allRooms = [...rooms, ...customRooms].filter(room => !overrides[room.id]?.hidden);
    ...
    return allRooms.map((room, idx) => {
        ...
        const alias = ov.alias ?? room.alias ?? null;
