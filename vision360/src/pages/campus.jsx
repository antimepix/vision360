...
export default function Campus() {
  ...
  const buildRooms = useCallback((floorKey) => {
    ...
    return buildRoomData(baseRooms, overrides, events, floorKey);
  }, []);
  ...
  const handleAddRoom = () => {
    ...
    overrides[newId] = newRoomConfig;
    saveOverrides(overrides);
    // Trigger re-render by calling buildRooms with the current floor
    setRooms(buildRooms(selectedFloor));
  };
  ...
  const handleDeleteRoom = (roomId) => {
    ...
    if (room.isNew) {
      ...
      overrides[roomId] = { ...overrides[roomId], hidden: true };
    }
  ...
  const handlePointerDown = (e, room, type = "move") => {
    ...
    const roomCenterX = rect.left + (room.xPercent + room.widthPercent / 2) * (rect.width / 100);
    const roomCenterY = rect.top + (room.yPercent + room.heightPercent / 2) * (rect.height / 100);
  ...
  const handlePointerMove = useCallback((e) => {
    ...
    setRooms((prev) => prev.map((r) => {
      ...
      if (dragging.type === "move") {
        ...
        const angleStart = Math.atan2(dragging.startMouseY - dragging.roomCenterY, dragging.startMouseX - dragging.roomCenterX);
        const angleCurrent = Math.atan2(clientY - dragging.roomCenterY, clientX - dragging.roomCenterX);
        const angleDiff = (angleCurrent - angleStart) * (180 / Math.PI);
        return { ...r, rotation: (dragging.startRotation + angleDiff) % 360 };
      }
      return r;
    }));
  }, [dragging]);
    ...
    setRooms((prev) => {
      ...
      if (updated) {
        ...
        overrides[updated.id] = {
          ...
          rotation: Math.round((updated.rotation || 0) * 100) / 100,
        };
        saveOverrides(overrides);
      }
      return prev;
